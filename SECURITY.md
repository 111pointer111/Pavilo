...
Pavilo 当前默认仍设计用于**可信网络环境**（内网、VPN 或配置了适当访问控制的代理后）。最新稳定版是 v1.4.0。值班台人员治理管的是临时席位；宿主 JWT 是可选接入端口，不等于完整社区治理。嵌入 SDK 属于 [v2.0 路线](ROADMAP.md)，尚未交付。
...
**主分支已实现、尚未发布（不构成 v1.4.0 的部署保证）：**

- 举报、墓碑式内容移除、`moderation.userDenyList`、值班台操作记录。见 [ADR-0011](docs/adr/0011-governance-loop.md)
...
- 使用反向代理 + 基础认证
- 使用 VPN 限制访问
- 宿主身份与频道授权按路线图 v1.4 阶段建设，当前不能依赖尚未实现的功能
...
- **v1.5**：举报、墓碑移除、稳定用户拒绝、操作记录。主分支已实现，尚未发布。不含嵌入，也不含提交前审核。
- **v1.6**：嵌入预览。浏览器检查改为阻塞发布。
- **v1.7**：同步文本规则。通用 Policy 端口和视觉审核更晚，而且不是 v2.0 门槛。
- **v2.0**：先做必要架构迁移，再冻结接入契约。

具体边界见 [集成设计](docs/integration.md)。嵌入只对指定入口开放来源，后台继续禁止嵌入；凭证不进 URL，不依赖第三方 Cookie；签名、签发者、受众、有效期和退出/续期需要验证。权限覆盖频道发现、历史和操作，不能只鉴权首页。
