...
> 状态：描述当前聊天状态契约。v1.6.0 已有 memory/sqlite 两种存储、临时席位禁言/请离、可选宿主 `userKey`（不进公开 roster），以及墓碑消息、举报和稳定用户拒绝。`/embed` 使用同一套会话，只是不把恢复令牌放进 `sessionStorage`。提交前审核见 [架构演进](../evolution.md)。这些契约必须在修改时保持兼容，或在独立变更中明确升级版本。

## 1. 生命周期与状态所有权

Pavilo 的运行时状态仍在单个服务进程内存中。ConversationStore 可以是 memory 或 sqlite：memory 重启即空；sqlite 保留历史真源，但不保存可直接复活的网络连接或登录会话。
...
当前默认驱动是 memory：没有数据库或跨进程恢复，服务实例结束时历史、身份、lease、回应和去重记录都结束。sqlite 模式下历史留在数据库，但身份、lease 和连接仍随进程结束。内核经 store 端口读写会话数据，不直接持有消息数组。
...
- `channelOccupancy` 替换所有频道的在线人数摘要，客户端不得从当前频道 roster 推断其他频道人数；
...
| `project` | 项目讨论 | 聚焦项目，高效协作。 | true | 32 |
