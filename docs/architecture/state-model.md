...
> 状态：描述当前聊天状态契约。v1.5.0 已有 memory/sqlite 两种存储、临时席位禁言/请离、可选宿主 `userKey`（不进公开 roster），以及墓碑消息、举报和稳定用户拒绝。嵌入会话与提交前审核见 [架构演进](../evolution.md)。这些契约必须在修改时保持兼容，或在独立变更中明确升级版本。
