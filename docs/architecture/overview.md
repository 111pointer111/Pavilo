...
本文档记录 Pavilo 当前架构的模块边界与职责划分。基础聊天采用结构化设计，不依赖构建工具、前端框架、数据库或新 WebSocket 库。配置默认值、v1/v2/v3/v4 兼容、CSS 和页面 DOM 保持既有契约。详见 [聊天协议](chat-protocol.md)、[状态模型](state-model.md)。
