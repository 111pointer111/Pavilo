...
> 产品与发布阶段补充（2026-09-22）：最新稳定版仍是 v1.4.0。主分支另有尚未发布的治理闭环。v2.0 另需覆盖嵌入授权、SDK 跨来源与生命周期、第三方 Cookie 禁用、完整狼人杀与迁移回滚。文本规则落地后再补审核竞态。当前 Node 24/26 和浏览器 CI 允许失败；嵌入那一版起，浏览器检查改为阻塞发布。完整门槛见 [路线图](../ROADMAP.md) 和 [版本管理](version-management.md)，不能用旧测试数量代替当次结果。
...
能力开关和宿主身份已随 v1.4.0 发布。治理闭环在主分支有测试，尚未发布。接下来按路线图补嵌入跨来源、提交前文本规则、完整玩法和升级回归；不再把早期 v0.2/v0.3 重构待办作为当前优先级。
