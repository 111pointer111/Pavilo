...
function harness(options = {}) {
  ...
  function element(name) {
    ...
    return {
      ...
      emit(type, payload = {}) {
        const event = { defaultPrevented: false, preventDefault() { this.defaultPrevented = true; }, ...payload };
        for (const handler of [...(callbacks.get(type) || [])]) handler(event);
        return event;
      },
    };
  }
  const elements = Object.fromEntries(['composer', 'composerText', 'sendButton', 'emojiButton', 'attachmentButton', 'imageInput',
    'replyingBar', 'replyingName', 'replyingText', 'cancelReplyButton'].map((name) => [name, element(name)]));
...
test('UMD loads without DOM/browser scheduling and exposes identical APIs', () => {
  ...
  assert.deepEqual(Object.keys(harness().composer).sort(), ['bind', 'clearReply', 'imageProcessingCount', 'sendImage', 'sendText', 'setReply', 'stopTyping', 'unbind', 'update']);
});
...
test('reply UI uses textContent and the unchanged image label, and cancel clears state', () => {
  ...
  room.composer.setReply({ ...reply, kind: 'image' });
...
test('image input and attachment handlers bind once and clear the selected input immediately', async () => {
  const room = harness();
...
test('image preparation locks buttons, not text editing, then creates a pending image and clears reply', async () => {
  const processing = deferred();
  const room = harness({ draft: 'draft', prepare: () => processing.promise });
  room.composer.setReply(reply);
  const sending = room.composer.sendImage(photo);
  ...
  assert.equal(item.epoch, 'epoch-one');
  assert.deepEqual(room.commands, [{ type: 'message', kind: 'image', clientMessageId: item.id, replyTo: reply.id, image: room.image }]);
  assert.equal(room.reply, null);
  assert.equal(room.elements.composerText.value, 'draft');
  assert.equal(room.composer.imageProcessingCount, 0);
  assert.equal(room.elements.sendButton.disabled, false);
...
test('unsupported/missing images and locked/full queues retain exact rejection strings', async () => {
  const missing = harness();
  assert.equal(await missing.composer.sendImage(), false);
  ...
  for (const [file, setup, message] of [
    ...
    [photo, (room) => { for (let index = 0; index < MAX_PENDING; index += 1) room.pending.add({ kind: 'image' }); }, '待确认的图片较多，请稍后再试。'],
  ]) {
    const room = harness();
    setup(room);
    assert.equal(await room.composer.sendImage(file), false);
    assert.deepEqual(room.toasts, [[message, true]]);
  ...
  for (const type of ['image/png', 'image/jpeg', 'image/gif', 'image/webp']) {
    const room = harness();
    assert.equal(await room.composer.sendImage({ type, size: 1 }), true);
    assert.equal(room.prepared[0].type, type);
  }
});

test('encoded image byte budget accepts exactly 4MB and ignores source file size', async () => {
  for (const [bytes, accepted] of [[MAX_PENDING_IMAGE_BYTES - 3, true], [MAX_PENDING_IMAGE_BYTES - 2, false]]) {
    ...
    assert.equal(await room.composer.sendImage(photo), accepted);
...
test('async preparations recheck both item count and byte budget before enqueueing', async () => {
  const processing = deferred();
  const room = harness({ prepare: () => processing.promise });
  for (let index = 0; index < MAX_PENDING - 1; index += 1) room.pending.add({ kind: 'image', status: 'accepted' });
  const first = room.composer.sendImage(photo);
  const second = room.composer.sendImage(photo);
  assert.equal(room.composer.imageProcessingCount, 2);
  processing.resolve(room.image);
  assert.equal(await first, true);
  assert.equal(await second, false);
  assert.equal(room.pending.size, MAX_PENDING);
  assert.deepEqual(room.toasts, [['待确认的图片较多，请稍后再试。', true]]);
  assert.equal(room.composer.imageProcessingCount, 0);
});

test('channel, epoch or connection changes during image preparation discard results and retain reply', async () => {
  for (const change of [
...
test('image send failure remains unconfirmed, unlike text failure', async () => {
  ...
  room.composer.setReply(reply);
  room.setSendResult(false);
  assert.equal(await room.composer.sendImage(photo), true);
  const item = [...room.pending.values()][0];
...
test('preparation errors restore all controls, preserve reply, and explain the failure', async () => {
  for (const [code, message] of [
    ...
    room.composer.setReply(reply);
    assert.equal(await room.composer.sendImage(photo), false);
    assert.deepEqual(room.toasts, [[message, true]]);
    assert.equal(room.composer.imageProcessingCount, 0);
    assert.equal(room.elements.sendButton.disabled, false);
    assert.equal(room.elements.attachmentButton.disabled, false);
