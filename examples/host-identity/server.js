...
const server = http.createServer((request, response) => {
  ...
  if (request.method === 'POST' && request.url === '/login') {
    ...
    request.on('end', () => {
      ...
      json(response, 200, { ok: true, identityToken, username: user.name, channels: user.channels, pavilo: PAVILO_URL });
    });
    return;
  }
...
server.listen(PORT, '127.0.0.1', () => {
  process.stdout.write(`host-identity example on http://127.0.0.1:${PORT}\n`);
});
