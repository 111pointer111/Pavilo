...
- 文字、表情、图片（PNG / JPEG / GIF / WebP，默认单张上限 1.5 MB）；表情选择器支持搜索、肤色和中文关键词
...
**消息 ACK** 表示“本次服务进程已接受”，不表示所有成员已送达或已读。默认容量包括：每频道最多 300 条历史、约 32 MB 消息预算、单图 1.5 MB、全局 64 名成员、80 个连接、同一 IP 12 个连接，以及慢连接写缓冲上限；超限时优先淘汰最旧消息。
